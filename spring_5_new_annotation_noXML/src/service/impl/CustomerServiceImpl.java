 ...
 * 客户的业务层实现类
 * 
 * 如何选择用哪种方式创建bean对象(注解还是xml)
 * 1.如果是web项目，修改注解需要修改源码，会导致需要重启服务器。prefer xml
 * 2.如果是大型项目，使用xml会导致配置文件过多(每个class都需要一个配置文件)。 prefer 注解
 * 3.xml的配置比注解略复杂
 * 4.如果需要经常修改配置，使用XML。如果配置不经常修改，而且配置文件很多，选annotation.
 * 
 * 注解
 * 1.用于创建bean对象
 * 		@Component
